...
Be better 是一个 **AI 掌握度引擎（Mastery Engine）**，当前以「大学英语四级阅读、写作与翻译」为试验场景。核心流程是：**学一小节 → 脱稿复述 → AI 按要点诊断 → 定点补讲 → 再讲一遍 → 可选课后例题与间隔复习**。课程可自由选择，掌握度用于反馈与复习提醒。
...
- **20 节原创图文微课**：阅读 12 节覆盖三类题型；写作 4 节、翻译 4 节按主题讲解。每节将常用必会与拔高选学分开展示。
...
- **可选课后例题**：阅读每节两道原创选择题，先作答，再讲选择与证据或直接看解析；写作、翻译每节三道原创开放题，共 24 道，先写英文答案，再获取 AI 反馈或直接看参考写法与解析。题目与原答案保留，便于重写。
...
5. **入口刻意做窄。**
   "让人变得更好"是长期愿景。项目从四级阅读起步，现已扩展到写作与翻译：同样先学、复述、诊断，再迁移到选择题或开放题。以后还可以将掌握度引擎用于其他知识或动作型领域，但需要针对任务重新设计诊断器。
...
- 我定义课程目标、rubric 结构和评判规则，并与 AI 协作编写微课和原创练习、检查内容；
...
本项目为个人学习与产品实践作品，与四六级考试官方无关。课程参考公开试卷和考试大纲，课后例题均为原创练习，不收录真题原文或第三方视频；写作与翻译参考写法不是唯一正确答案，AI 反馈也不是官方评分。资料链接与范围见 [CONTENT_SOURCES.md](CONTENT_SOURCES.md)。本产品不保证记忆保持或考试结果，完整备考仍需结合正式真题和限时训练。
