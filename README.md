...
> 用最短的内容教你一件事，再让你讲给它听、做给它看，直到你真的掌握。

Be better 是一个 **AI 掌握度引擎（Mastery Engine）**，而不是又一门网课、题库或 AI 问答。当前以「大学英语四级阅读」为第一个试验场景，验证一个核心闭环：**先学一小节 → 脱稿复述 → AI 按评分要点诊断 → 只补你漏的那几点 → 再讲一遍 → 达标才解锁**。
...
| **Be better** | **学 → 讲 → 诊断 → 定点补讲 → 再讲 → 达标解锁** | **AI 按 rubric 判定掌握度，并驱动你补到会为止** |
...
flowchart LR
  ...
  D --> E[清空、脱稿再讲一遍]
  E --> C
  C -->|覆盖率≥70% 且无硬伤| F[解锁下一节]
  F -.规划中.-> G[第 2 天 / 第 7 天间隔复述]
```

## 四、当前已实现

- **5 节原创图文微课**：覆盖四级仔细阅读的主旨大意、细节定位、推理判断、观点态度与词义猜测，以及选词填空（篇章词汇）。每节含本节目标、分块短图文、方法提示和一道方法演示例题。
- **脱稿复述**：学完进入复述页，用自己的话把方法讲一遍（当前为文本，语音复述在路线图中）。
- **AI 诊断**：服务端调用大模型，按每节预置的 5 个评分要点判定覆盖、遗漏与硬伤错误，返回覆盖率、错误清单、鼓励和参考讲法。
- **定点补讲**：不通过时，AI 只围绕本次遗漏点和错误生成补讲卡片（思路 + 小例子 + 记忆提示），完整参考答案默认折叠，避免照抄；清空后脱稿再讲，直到达标。
- **掌握度门槛与解锁**：覆盖率 ≥ 70% 且硬伤错误数为 0 才判定通过并解锁下一节。
- **本地进度**：完成状态、最佳覆盖率、尝试次数和每次诊断结果保存在浏览器 `localStorage`，刷新不丢失。
...
2. **模型只做语义判断，分数由程序重算，不信任模型自报。**
   大模型负责判断"这段话有没有覆盖某个要点"，但覆盖率、是否通过一律在服务端和前端**各重算一次**；通过条件写死为「覆盖率 ≥ 0.7 且致命错误数为 0」。这样即使模型返回了不合规的 JSON 或自相矛盾的分数，也不会让用户错误过关。模型输出在服务端做了 id 白名单过滤、字段归一化和失败重试。
...
4. **AI 可能把"说得像"误判成"真的懂"，所以标准要可校准。**
   rubric 锚定权威解题方法而非模型自由发挥；路线图里会加入"同题型迁移小题"和人工标注样本校准，防止用户靠堆关键词刷过。
...
app/
  ...
  me/page.tsx                # 我的进度
lib/
  lessons.ts                 # 5 节微课内容 + 每节 rubric / 常见错误 / 参考讲法
...
- [x] 5 节四级阅读图文微课 + 费曼复述 + rubric 诊断 + 达标解锁
- [x] 针对遗漏点的定点补讲，不直接给完整答案
- [ ] 第 2 天 / 第 7 天间隔复述（对抗遗忘，验证长期留存）
- [ ] 同题型迁移小题与追问，防止背关键词刷过
- [ ] 语音复述（浏览器语音识别，支持"用嘴讲一遍"）
- [ ] 课程扩充到 10–20 节，并引入人工校准 AI 评分
- [ ] 把掌握度引擎迁移到一个动作型场景（如深蹲，需姿态关键点诊断）

## 十一、内容与免责说明

本项目为个人学习与产品实践作品，与四六级考试官方无关。课程内容为基于公开真题、官方答案解析与通用考纲的成熟解题方法所做的原创改写，不收录任何受版权保护的真题原文或第三方视频。备考请以官方教材与考试要求为准。
